...
constexpr size_t IDT_ENTRIES_COUNT = 256;

static struct idt_gate entries[IDT_ENTRIES_COUNT];
...
static void pic_remap() {
    ...
    const struct pic seq[] = {{0x20, 0x11}, {0xA0, 0x11}, {0x21, 0x20}, {0xA1, 0x28}, {0x21, 0x04},
                              {0xA1, 0x02}, {0x21, 0x01}, {0xA1, 0x01}, {0x21, 0x00}, {0xA1, 0x00}};

    for (auto i = 0u; i < sizeof(seq) / sizeof(seq[0]); i++) {
        outb(seq[i].port, seq[i].val);
        waitb(1);
    }

    qemu_printf(QEMU_CPU, QEMU_OK, "PIC remapped");
}

static void irq_setup() {
    const void *handlers[] = {irq0, irq1, irq2,  irq3,  irq4,  irq5,  irq6,  irq7,
...
static void idt_setup() {
    const void *handlers[] = {isr0,  isr1,  isr2,  isr3,  isr4,  isr5,  isr6,  isr7,  isr8,  isr9,  isr10,
