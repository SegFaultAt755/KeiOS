...
void paging_set_physical_memory(uint32_t mem_high_point, uint32_t physical_alloc_start) {
    qemu_printf(QEMU_INFO, "Calculating page frames");
    ...
    qemu_printf(QEMU_INFO, "Scale to array with size %d", sizeof(physical_mem_bitmap));
...
void paging_initialize(uint32_t mem_high_point, uint32_t physical_alloc_start) {
    qemu_printf(QEMU_INFO, "Resetting first point from initial page directories");
    ...
    qemu_printf(QEMU_INFO, "Recursive mapping from the end");
    ...
    qemu_printf(QEMU_INFO, "Setting physical memory");
    invalidate(PAGING_KERNEL_END);
    paging_set_physical_memory(mem_high_point, physical_alloc_start);

    qemu_printf(QEMU_INFO, "Resetting page directories");
