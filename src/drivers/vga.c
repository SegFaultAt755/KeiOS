...
bool vga_is_graphics_supported(void) {
    qemu_printf(QEMU_LOG_INFO, "Getting if video mode is supported {%d}\n", __FILE__, __LINE__, video_mode_enabled);
...
void vga_clear_screen(uint32_t color) {
    ...
    if (video_mode_enabled) {
        qemu_printf(QEMU_LOG_INFO, "Clearing screen for video mode {%dx%d}\n", gfx_width, gfx_height);
...
void vga_set_pixel(uint32_t x, uint32_t y, uint32_t color) {
    if (!video_mode_enabled || x >= gfx_width || y >= gfx_height) {
        qemu_printf(QEMU_LOG_ERROR, "(%s:%d) Failed to set pixel: {%d, %d:%d:%d, %d:%d:%d}\n", __FILE__, __LINE__,
