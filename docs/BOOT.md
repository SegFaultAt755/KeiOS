...
`````txt
[Firmware & Bootloader] ---> Multiboot2 bootloader [DONE]
...
2. **GRUB2 Handshake:** GRUB reads `grub.cfg`, switches the CPU to 32-bit Protected Mode, and parses the Multiboot2 header
...
1. **Verify Multiboot Header:** Confirm `EAX` contains the correct Multiboot2 magic number
...
2. **Memory Subsystems:**
    - **Physical Memory Manager:** Parse the Multiboot2 memory map tag to initialize the physical page bitmap
